...
#Region "Constant"
    ...
    Public Const GridColAuthSelectCtrl  As String = "checkBoxLinesSelected"
...
#Region "Page Internal"
    ...
    Private Sub UpdatePoLinesRecordCount(records As Integer)
        ...
        End If
    End Sub
    Private Function GetSelectedClaimIds() As List(Of Guid)
        Dim chkbox As CheckBox, claimIdStr As String
        Dim selectedClaims As List(Of Guid) = New List(Of Guid)

        For Each row As GridViewRow In GridAuth.Rows
            If row.RowType = DataControlRowType.DataRow Then
                chkbox = CType(row.Cells(GridColCheckBoxIdx).FindControl(GridColClaimSelectCtrl), CheckBox)
                If chkbox.Checked Then
                    claimIdStr = row.Cells(GridColClaimIdx).Text.Trim
                    If String.IsNullOrEmpty(claimIdStr) = False Then
                        selectedClaims .Add(New Guid(claimIdStr))
                    End If
                End If
            End If
        Next
    ...
    Private Function GetSelectedAuthorizationIds() As List(Of Guid)
        ...
        For Each row As GridViewRow In GridAuth.Rows
            If row.RowType = DataControlRowType.DataRow Then
                chkbox = CType(row.Cells(GridColCheckBoxIdx).FindControl(GridColClaimSelectCtrl), CheckBox)
                If chkbox.Checked Then
                    claimIdStr = row.Cells(GridColAuthorizationIdx).Text.Trim
                    If String.IsNullOrEmpty(claimIdStr) = False Then
...
#Region "Authorization Grid"
   ...
   Private Function GetAuthorizationGridDataView() As DataView
        ...
        Return apInvoiceLinesBo.GetAuthorization(ServiceCenterId, txtClaimNumber.Text, txtAuthorizationNumber.Text,CompanyId)
    End Function
    ...
    Private Sub GridAuth_PageIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles GridAuth.PageIndexChanged
        ...
        End Try
    End Sub
    Private Sub Grid_PageIndexChanging(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewPageEventArgs) Handles GridAuth.PageIndexChanging
        Try
    ...
    Private Sub Grid_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles GridAuth.RowDataBound
        ...
        Try
            If e.Row.RowType = DataControlRowType.DataRow Then
                ElitaHostPage.PopulateControlFromBOProperty(e.Row.Cells(GridColAuthorizationIdx), GetGuidStringFromByteArray(dvRow(GridColAuthorizationDataField)))
            End If
        Catch ex As Exception
            HandleLocalException(ex)
        End Try
...
#Region "Po Line Grid"
    ...
    Private Sub GridPoLines_PageIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles GridPoLines.PageIndexChanged
        ...
        End Try
    End Sub
    Private Sub GridPoLines_PageIndexChanging(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewPageEventArgs) Handles GridPoLines.PageIndexChanging
        Try
