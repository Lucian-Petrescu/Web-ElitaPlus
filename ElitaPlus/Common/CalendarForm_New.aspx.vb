...
#End Region
    ...
    Private _enabledDateList as List(of string)
    Private _disableBeforeDate as string
#End Region
...
#Region " Form Related "
    ...
    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Dim enabledDates As String = HttpContext.Current.Request.QueryString("enabledDates")
        If string.IsNullOrEmpty(enabledDates) = False Then
            _enabledDateList = enabledDates.Split("|").ToList()
        End If

        _disableBeforeDate = HttpContext.Current.Request.QueryString("disableBeforeDate")
    ...
    Private Sub MyCalendar_SelectionChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyCalendar.SelectionChanged
        ...
        If fieldNameParam.IndexOf(":") < 0 Then
            ...
            sJscript &= " var element = window.opener.document.getElementById('" & HttpContext.Current.Request.QueryString("formname") & "'); if(element.onchange) { try { element.onchange(); } catch(err){} } " & Environment.NewLine
...
#End Region
    Protected Sub MyCalendar_DayRender(sender As Object, e As DayRenderEventArgs) Handles MyCalendar.DayRender
        if (not _enabledDateList Is Nothing) AndAlso _enabledDateList.Count > 0 Then
            ...
            End If
        elseif string.IsNullOrEmpty(_disableBeforeDate ) = false then 
            'disable all days prior the incoming date
            if _disableBeforeDate >  e.Day.Date.ToString("yyyyMMdd") then
                e.Day.IsSelectable = False
                e.Cell.ForeColor = Color.Gray
                e.Cell.Font.Strikeout = True
            End If
