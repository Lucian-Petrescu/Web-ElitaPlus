...
#Region "Helper functions"
    ...
    Protected Sub BindBoPropertiesToGridHeaders()
        ...
        BindBOPropertyToGridHeader(State.MyBO, "StoragePath", Grid.Columns(GRID_COL_STORAGE_PATH_IDX))
        BindBOPropertyToGridHeader(State.MyBO, "RepositoryType", Grid.Columns(GRID_COL_REPOSITORY_TYPE_IDX))
    ...
    Private Function PopulateBOFromForm() As Boolean
        ...
        With State.MyBO
            ...
            .StoragePath = CType(Grid.Rows((Grid.EditIndex)).Cells(GRID_COL_STORAGE_PATH_IDX).FindControl(GRID_CTRL_NAME_EDIT_STORAGE_PATH), TextBox).Text.Trim.ToUpper
            .RepositoryTypeXcd = CType(Grid.Rows((Grid.EditIndex)).Cells(GRID_COL_REPOSITORY_TYPE_IDX).FindControl(GRID_CTRL_NAME_EDIT_REPOSITORY_TYPE), TextBox).Text.Trim.ToUpper
        End With
...
#Region "Grid related"
    ...
    Protected Sub cboPageSize_SelectedIndexChanged(sender As System.Object, e As System.EventArgs) Handles cboPageSize.SelectedIndexChanged
        Try
            ...
            State.PageIndex = NewCurrentPageIndex(Grid, State.RepositoryList.Count(), State.PageSize)
    ...
    Private Sub Grid_RowDataBound(sender As Object, e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles Grid.RowDataBound
        Try
            ...
            If ((e.Row.RowState And DataControlRowState.Edit) = DataControlRowState.Edit) Then
                ...
                CType(e.Row.Cells(GRID_COL_STORAGE_PATH_IDX).FindControl(GRID_CTRL_NAME_EDIT_STORAGE_PATH), TextBox).Text = ft.StoragePath
                CType(e.Row.Cells(GRID_COL_REPOSITORY_TYPE_IDX).FindControl(GRID_CTRL_NAME_EDIT_REPOSITORY_TYPE), TextBox).Text = ft.RepositoryTypeXcd
                ...
                CType(e.Row.Cells(GRID_COL_STORAGE_PATH_IDX).FindControl(GRID_CTRL_NAME_EDIT_STORAGE_PATH), TextBox).ReadOnly = Not State.IsGridAddNew
                CType(e.Row.Cells(GRID_COL_REPOSITORY_TYPE_IDX).FindControl(GRID_CTRL_NAME_EDIT_REPOSITORY_TYPE), TextBox).ReadOnly = Not State.IsGridAddNew
            ...
            CType(e.Row.Cells(GRID_COL_DESCRIPTION_IDX).FindControl(GRID_CTRL_NAME_LABLE_DESCRIPTION), Label).Text = ft.Description
                CType(e.Row.Cells(GRID_COL_STORAGE_PATH_IDX).FindControl(GRID_CTRL_NAME_LABEL_STORAGE_PATH), Label).Text = ft.StoragePath
                CType(e.Row.Cells(GRID_COL_BUTTON_IDX).FindControl(GRID_CTRL_NAME_BUTTON_EDIT), ImageButton).CommandArgument = ft.Id.ToString()
                CType(e.Row.Cells(GRID_COL_REPOSITORY_TYPE_IDX).FindControl(GRID_CTRL_NAME_LABEL_REPOSITORY_TYPE), Label).Text = ft.RepositoryTypeXcd
            End If
        ...
        Try
            ...
            Select Case e.CommandName
                Case EDIT_COMMAND
                    ...
                    State.MyBO = State.RepositoryList.Where(Function(ft) ft.Id = State.RepositoryId).First()
                ...
                Case CANCEL_COMMAND
                    With State
                        If .IsGridAddNew Then
                            If (State.RepositoryList.Contains(State.MyBO)) Then
                                State.RepositoryList.Remove(State.MyBO)
                            End If
                            .IsGridAddNew = False
    ...
    Private Function GetSortKey(pRepository As Repository) As String
        ...
        ElseIf (SortDirection.Contains("STORAGE_PATH")) Then
            Return pRepository.StoragePath
        ElseIf (SortDirection.Contains("REPOSITORY_TYPE")) Then
    ...
    Public Sub PopulateGrid()
        ...
        Try
            ...
            With State
                If (.RepositoryList Is Nothing) Then
                    .RepositoryList = _
                        ...
                        .Where(Function(r) _
                                   ((State.SearchCode Is Nothing) OrElse (State.SearchCode.Trim().Length = 0) OrElse (r.Code.Contains(State.SearchCode))) AndAlso _
                                   ((State.SearchDescription Is Nothing) AndAlso (State.SearchDescription.Trim().Length = 0) OrElse (r.Description.Contains(State.SearchDescription)))))
            ...
            If (SortDirection.EndsWith("DESC")) Then
                State.RepositoryList = State.RepositoryList.OrderByDescending(Function(ft) GetSortKey(ft)).ToList()
            Else
                State.RepositoryList = State.RepositoryList.OrderBy(Function(ft) GetSortKey(ft)).ToList()
            End If

            If (State.IsAfterSave) Then
                State.IsAfterSave = False
                SetPageAndSelectedIndexFromGuid(Of Repository)( _
                    State.RepositoryList, _
                    Function(ft) ft.Id = State.RepositoryId, _
                    Grid, _
                    State.PageIndex)
    ...
    Private Sub SortAndBindGrid(Optional ByVal blnShowErr As Boolean = True)
        ...
        If (State.RepositoryList.Count = 0) Then
            State.MyBO = DocumentManager.Current.NewRepository()
            State.RepositoryList.Add(State.MyBO)
            Grid.DataSource = State.RepositoryList
            Grid.DataBind()
        ...
        Session("recCount") = State.RepositoryList.Count

        If Grid.Visible Then
            If (State.IsGridAddNew) Then
                lblRecordCount.Text = (State.RepositoryList.Count - 1) & " " & TranslationBase.TranslateLabelOrMessage(Message.MSG_RECORDS_FOUND)
            Else
                lblRecordCount.Text = State.RepositoryList.Count & " " & TranslationBase.TranslateLabelOrMessage(Message.MSG_RECORDS_FOUND)
            End If
        End If
...
#Region "Control Handler"
    ...
    Private Sub AddNew()
        If State.MyBO Is Nothing OrElse State.MyBO.IsNew = False Then
            State.MyBO = DocumentManager.Current.NewRepository()
            If (State.RepositoryList Is Nothing) Then
                State.RepositoryList = New List(Of Repository)(DocumentManager.Current.Repositories.AsEnumerable())
            End If
            State.RepositoryList.Add(State.MyBO)
        End If
        State.RepositoryId = State.MyBO.Id
        State.IsGridAddNew = True
        PopulateGrid()
        'Set focus on the Code TextBox for the EditItemIndex row
        SetPageAndSelectedIndexFromGuid(Of Repository)(
            State.RepositoryList,
            Function(ft) ft.Id = State.RepositoryId,
            Grid,
            State.PageIndex,
            State.IsEditMode)
        ControlMgr.DisableEditDeleteGridIfNotEditAuth(Me, Grid)
