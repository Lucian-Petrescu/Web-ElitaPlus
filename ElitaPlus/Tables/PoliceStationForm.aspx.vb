    ...
    Private Sub Page_PageCall(CallFromUrl As String, CallingPar As Object) Handles MyBase.PageCall
        Try
            If CallingParameters IsNot Nothing Then
                ...
                State.MyBO = New PoliceStation(CType(CallingParameters, Guid))
            Else
                State.IsNew = True
            End If
...
#Region "Page Return Type"
    Public Class ReturnType
        ...
        Public HasDataChanged As Boolean
        Public Sub New(LastOp As DetailPageCommand, curEditingBo As PoliceStation, hasDataChanged As Boolean)
...
#Region "Controlling Logic"
    ...
    Protected Sub BindBoPropertiesToLabels()
        BindBOPropertyToLabel(State.MyBO, POLICESTATIONCODE_PROPERTY, lblPoliceStationCode)
        BindBOPropertyToLabel(State.MyBO, POLICESTATIONNAME_PROPERTY, lblPoliceStationName)
        BindBOPropertyToLabel(State.MyBO, POLICESTATIONDISTRICTCODE_PROPERTY, lblPoliceStationDistrictCode)
        BindBOPropertyToLabel(State.MyBO, POLICESTATIONDISTRICTNAME_PROPERTY, lblPoliceStationDistrictName)
    ...
    Protected Sub PopulateFormFromBOs()
        With State.MyBO
            ...
            PopulateControlFromBOProperty(txtPoliceStationCode, .PoliceStationCode)
            PopulateControlFromBOProperty(txtPoliceStationName, .PoliceStationName)
            PopulateControlFromBOProperty(txtPoliceStationDistrictCode, .PoliceStationDistrictCode)
            PopulateControlFromBOProperty(txtPoliceStationDistrictName, .PoliceStationDistrictName)
    ...
    Protected Sub PopulateBOsFromForm()
        ...
        With State.MyBO
            ...
            PopulateBOProperty(State.MyBO, "PoliceStationCode", txtPoliceStationCode)
            PopulateBOProperty(State.MyBO, "PoliceStationName", txtPoliceStationName)
            PopulateBOProperty(State.MyBO, "PoliceStationDistrictCode", txtPoliceStationDistrictCode.Text())
            PopulateBOProperty(State.MyBO, "PoliceStationDistrictName", txtPoliceStationDistrictName.Text())
    ...
    Protected Sub CreateNewWithCopy()
        ...
        State.MyBO = newObj
        State.MyBO.PoliceStationCode = Nothing
        State.MyBO.PoliceStationName = Nothing
        State.MyBO.PoliceStationDistrictCode = Nothing
        State.MyBO.PoliceStationDistrictName = Nothing

        PopulateFormFromBOs()
        EnableDisableFields()

        'create the backup copy
        State.ScreenSnapShotBO = New PoliceStation
    ...
    Private Sub btnUndo_WRITE_Click(sender As Object, e As System.EventArgs) Handles btnUndo_WRITE.Click
        Try
            If Not State.MyBO.IsNew Then
                'Reload from the DB
                State.MyBO = New PoliceStation(State.MyBO.Id)
