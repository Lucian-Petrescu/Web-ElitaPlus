...
#Region "Load Methods"
    ...
    Public Sub Load(familyDS As DataSet, id As Guid, OmitText As Boolean)
        ...
        If OmitText Then
            selectStmt = Config("/SQL/LOAD_WITHOUT_TEXT")
        Else
    ...
    Public Function LoadList(CompanyId As Guid, StatusToInclude As ArrayList, LanguageId As Guid, Optional ByVal JournalOnly As Boolean = True) As DataSet
        Dim selectStmt As String = Config("/SQL/LOAD_LIST")
        Dim whereClauseConditions As String = ""
        Dim parameters() As OracleParameter = New OracleParameter() {New OracleParameter("language_id", LanguageId.ToByteArray)}

        whereClauseConditions &= String.Format(" AND {0} =HEXTORAW('{1}')", COL_NAME_COMPANY_ID, GuidToSQLString(CompanyId))
        ...
        selectStmt = selectStmt.Replace(DYNAMIC_ORDER_BY_CLAUSE_PLACE_HOLDER, " Order By created_date desc")

        Try
            'Dim ds = New DataSet
            Return (DBHelper.Fetch(selectStmt, DATASET_NAME, TABLE_NAME, parameters))
        Catch ex As Exception
            Throw New DataBaseAccessException(DataBaseAccessException.DatabaseAccessErrorType.ReadErr, ex)
        End Try
    ...
    Public Function LoadList(ParentFileName As String, ParentBatchNumber As String, RejectReason As String, StatusToOmit As Guid, LanguageId As Guid) As DataSet
        Dim selectStmt As String = Config("/SQL/LOAD_LIST")
        Dim whereClauseConditions As String = ""
        Dim parameters() As OracleParameter = New OracleParameter() {New OracleParameter("language_id", LanguageId.ToByteArray)}

        whereClauseConditions &= String.Format(" AND UPPER({0}) ='{1}'", COL_NAME_BATCH_NUMBER, ParentBatchNumber)
        whereClauseConditions &= String.Format(" AND NOT UPPER({0}) = UPPER('{1}')", COL_NAME_FILE_NAME, ParentFileName)

        If (Not (RejectReason Is Nothing)) AndAlso (RejectReason.Trim.Length > 0) Then
            whereClauseConditions &= String.Format(" AND UPPER({0}) = '{1}'", COL_NAME_REJECT_REASON, RejectReason.ToUpper)
        End If

        If Not StatusToOmit.Equals(Guid.Empty) Then
            whereClauseConditions &= String.Format(" AND NOT {0} = HEXTORAW('{1}') ", COL_NAME_STATUS_ID, GuidToSQLString(StatusToOmit))
        End If
    ...
    Public Function LoadList(CompanyId As Guid, StartDate As Date, EndDate As Date, BatchNumber As String, StatusToOmit As Guid, LanguageId As Guid) As DataSet
        Dim selectStmt As String = Config("/SQL/LOAD_LIST")
        Dim whereClauseConditions As String = ""
        Dim parameters() As OracleParameter = New OracleParameter() {New OracleParameter("language_id", LanguageId.ToByteArray)}

        whereClauseConditions &= String.Format(" AND {0} =HEXTORAW('{1}')", COL_NAME_COMPANY_ID, GuidToSQLString(CompanyId))
        whereClauseConditions &= String.Format(" AND {0} is null ", COL_NAME_REJECT_REASON)
        whereClauseConditions &= String.Format(" AND UPPER({0}) like '%JOURNAL%' ", COL_NAME_FILE_NAME)
        whereClauseConditions &= String.Format(" AND ELP_ACCT_TRANSMISSION.{0} > SYSDATE - {1} ", COL_NAME_CREATED_DATE, MAX_REVERSAL_DAYS)

        If (Not (BatchNumber Is Nothing)) AndAlso (BatchNumber.Trim.Length > 0) Then
            whereClauseConditions &= String.Format(" AND UPPER({0}) like '%{1}.XML'", COL_NAME_FILE_NAME, BatchNumber.ToUpper)
        Else
            If Not StartDate.Equals(Date.MinValue) Then
                whereClauseConditions &= String.Format(" AND ELP_ACCT_TRANSMISSION.{0} > TO_DATE('{1}','MM/DD/YYYY')", COL_NAME_CREATED_DATE, StartDate.ToString("MM/dd/yyyy"))
            End If
            If Not EndDate.Equals(Date.MinValue) Then
                whereClauseConditions &= String.Format(" AND ELP_ACCT_TRANSMISSION.{0} < TO_DATE('{1}','MM/DD/YYYY')", COL_NAME_CREATED_DATE, EndDate.ToString("MM/dd/yyyy"))
            End If
        End If
        ...
        selectStmt = selectStmt.Replace(DYNAMIC_ORDER_BY_CLAUSE_PLACE_HOLDER, " Order By ELP_ACCT_TRANSMISSION.created_date desc")
    ...
    Public Function LoadList(CompanyId As Guid, FileName As String, AcctTransmissionId As Guid) As DataSet
        Dim selectStmt As String = Config("/SQL/LOAD_TRANSMISSION_LIST")
...
#Region "XML Migration Methods"
    Public Sub MigrateXML(rowcount As Integer, modified_by As String)

        Dim readStmt As String = Config("/SQL/LOAD_UNCOMPRESSED_LIST")
        Dim updateStmt As String = Config("/SQL/UPDATE_COMPRESSED_XML")
    ...
    Public Function TransmissionsToMigrate() As Integer
        Dim stmt As String = Config("/SQL/TRANSMISSIONSTOMIGRATE")
...
#Region "Public Methods"
    ...
    Public Sub InsertCustom(CurrentRow As DataRow, CreatedBy As String)
        ...
        compressionmethod = CompressionProviderFactory.Current.CreateInstance(CompressionProviderType.IonicZip)
        Dim outputmemorystream As MemoryStream = New MemoryStream()
        compressionmethod.Compress(CurrentRow(COL_NAME_FILE_TEXT).ToString(), outputmemorystream)

        Dim parameters() As DBHelper.DBHelperParameter = New DBHelper.DBHelperParameter() _
                            {New DBHelper.DBHelperParameter(COL_NAME_FILE_NAME, CurrentRow(COL_NAME_FILE_NAME).ToString), _
                            New DBHelper.DBHelperParameter(COL_NAME_FILE_TEXT_COMPRESSED, outputmemorystream.ToArray()), _
                            New DBHelper.DBHelperParameter(COL_NAME_TRANSMISSION_DATE, If(IsDate(CurrentRow(COL_NAME_TRANSMISSION_DATE)), CurrentRow(COL_NAME_TRANSMISSION_DATE), Date.MinValue)), _
                            New DBHelper.DBHelperParameter(COL_NAME_TRANSMISSION_COUNT, Integer.Parse(CurrentRow(COL_NAME_TRANSMISSION_COUNT))), _
                            New DBHelper.DBHelperParameter(COL_NAME_DEBIT_AMOUNT, If(IsDBNull(CurrentRow(COL_NAME_DEBIT_AMOUNT)), 0, CurrentRow(COL_NAME_DEBIT_AMOUNT))), _
                            New DBHelper.DBHelperParameter(COL_NAME_CREDIT_AMOUNT, If(IsDBNull(CurrentRow(COL_NAME_CREDIT_AMOUNT)), 0, CurrentRow(COL_NAME_CREDIT_AMOUNT))), _
                            New DBHelper.DBHelperParameter(COL_NAME_REJECT_REASON, CurrentRow(COL_NAME_REJECT_REASON).ToString), _
                            New DBHelper.DBHelperParameter(COL_NAME_COMPANY_ID, CType(CurrentRow(COL_NAME_COMPANY_ID), Byte())), _
                            New DBHelper.DBHelperParameter(COL_NAME_CREATED_BY, CreatedBy), _
                            New DBHelper.DBHelperParameter(COL_NAME_FILE_TYPE_FLAG, Integer.Parse(CurrentRow(COL_NAME_FILE_TYPE_FLAG))), _
                            New DBHelper.DBHelperParameter(COL_NAME_FILE_SUB_TYPE_FLAG, Integer.Parse(CurrentRow(COL_NAME_FILE_SUB_TYPE_FLAG))), _
                            New DBHelper.DBHelperParameter(COL_NAME_NUM_TRANSACTIONS_SENT, If(IsDBNull(CurrentRow(COL_NAME_NUM_TRANSACTIONS_SENT)), 0, Integer.Parse(CurrentRow(COL_NAME_NUM_TRANSACTIONS_SENT)))), _
                            New DBHelper.DBHelperParameter(COL_NAME_DEBIT_AMOUNT_RECEIVED, If(IsDBNull(CurrentRow(COL_NAME_DEBIT_AMOUNT_RECEIVED)), 0, CurrentRow(COL_NAME_DEBIT_AMOUNT_RECEIVED))), _
                            ...
                            New DBHelper.DBHelperParameter(COL_NAME_JOURNAL_TYPE, CurrentRow(COL_NAME_JOURNAL_TYPE).ToString)}
    ...
    Public Sub DeleteCustom(AcctTransmissionId As Guid, DeletedStatusId As Guid)

        Dim selectStmt As String = Config("/SQL/DELETE")
        Dim parameters() As DBHelper.DBHelperParameter = New DBHelper.DBHelperParameter() _
                           {New DBHelper.DBHelperParameter(COL_NAME_STATUS_ID, DeletedStatusId.ToByteArray), _
                           New DBHelper.DBHelperParameter(COL_NAME_ACCT_TRANSMISSION_ID, AcctTransmissionId.ToByteArray)}
