...
#Region "CRUD Methods"
    ...
    Public Function LoadList(code As String, _
        ...
        If (Not (effective Is String.Empty)) Then
            whereClauseConditions &= " AND " & Environment.NewLine & "(" & COL_NAME_EFFECTIVE & ")" & " =  to_date('" & DateHelper.GetDateValue(effective).ToString("MM/dd/yyyy HH:mm:ss") & "', 'mm-dd-yyyy hh24:mi:ss')"
            bIsWhereClause = True
        End If
    ...
    Public Sub LoadEQ(DS As DataSet, id As Guid)
        Dim selectStmt As String = Config("/SQL/LOAD_EQ_LIST")
        Dim parameters() As OracleParameter = New OracleParameter() {New OracleParameter("equipment_list_id", id.ToByteArray)}
        Try
    ...
    Public Function CheckOverlap(code As String, _
        ...
        End If
        If (Not (effective Is String.Empty)) Then
            whereClauseConditions &= " AND " & Environment.NewLine & "TO_DATE('" & DateHelper.GetDateValue(effective).ToString("MM/dd/yyyy HH:mm:ss") & "', 'mm-dd-yyyy hh24:mi:ss')" & " >=  (SELECT MAX(EFFECTIVE) FROM ELP_EQUIPMENT_LIST WHERE UPPER(" & COL_NAME_CODE & ")" & code.ToUpper & ")"
            bIsWhereClause = True
        End If
        If bIsWhereClause Then
            selectStmt = selectStmt.Replace(DYNAMIC_WHERE_CLAUSE_PLACE_HOLDER, whereClauseConditions)
        ...
        EquipmentListIdParam = New DBHelper.DBHelperParameter(COL_NAME_EQUIPMENT_LIST_ID, listId.ToByteArray)
    ...
    Public Function CheckOverlapToExpire(code As String, _
        ...
        Try
            ...
            End If
            If (Not (effective Is String.Empty)) Then
                whereClauseConditions &= " AND " & Environment.NewLine & "TO_DATE('" & DateTime.Parse(effective).ToString("MM/dd/yyyy HH:mm:ss") & "', 'mm-dd-yyyy hh24:mi:ss')" & " >=  (SELECT MAX(EFFECTIVE) FROM ELP_EQUIPMENT_LIST WHERE UPPER(" & COL_NAME_CODE & ")" & code.ToUpper & ")"
                bIsWhereClause = True
            End If
            If bIsWhereClause Then
                selectStmt = selectStmt.Replace(DYNAMIC_WHERE_CLAUSE_PLACE_HOLDER, whereClauseConditions)
    ...
    Public Function ExpireList(PrevListDS As DataSet, effective As String) As Boolean
        ...
        Try
            For index = 0 To PrevListDS.Tables(0).Rows.Count - 1
                EquipmentListIdParam = New DBHelper.DBHelperParameter(COL_NAME_EQUIPMENT_LIST_ID, PrevListDS.Tables(0).Rows(index)(COL_NAME_EQUIPMENT_LIST_ID))
                EquipmentExpiration = New DBHelper.DBHelperParameter(COL_NAME_EXPIRATION, DateHelper.GetDateValue(effective).AddMinutes(-1))
                DBHelper.Fetch(ds, selectStmt, TABLE_NAME, New DBHelper.DBHelperParameter() {EquipmentExpiration, EquipmentListIdParam})
                ExpireListEquipments(PrevListDS.Tables(0).Rows(index)(COL_NAME_EQUIPMENT_LIST_ID), DateHelper.GetDateValue(effective).AddMinutes(-1).ToString)
            Next
        Catch ex As Exception
            Throw New DataBaseAccessException(DataBaseAccessException.DatabaseAccessErrorType.ReadErr, ex)
        End Try
    ...
    Public Function ExpireListEquipments(equipmentListId As Byte(), expiration As String) As Boolean

        Dim selectStmt As String = Config("/SQL/EXPIRE_EQUIPMENTS")
        Dim ds As New DataSet
        Dim EquipmentListIdParam As DBHelper.DBHelperParameter
        Dim whereClauseConditions As String = String.Empty
        ...
        Try
            EquipmentListIdParam = New DBHelper.DBHelperParameter(COL_NAME_EQUIPMENT_LIST_ID, equipmentListId)
            DBHelper.Fetch(ds, selectStmt, TABLE_NAME, New DBHelper.DBHelperParameter() {EquipmentListIdParam})
        Catch ex As Exception
            Throw New DataBaseAccessException(DataBaseAccessException.DatabaseAccessErrorType.ReadErr, ex)
        End Try
    ...
    Public Function IsEquipmentInList(equipmentId As Guid, equipmentListId As Guid, companyIds As ArrayList, languageId As Guid) As DataSet

        Dim selectStmt As String = Config("/SQL/IS_EQUIPMENT")
        Dim ds As New DataSet
        ...
        Try
            ...
            EquipmentListIdParam = New DBHelper.DBHelperParameter(COL_NAME_EQUIPMENT_LIST_ID, equipmentListId)
            DBHelper.Fetch(ds, selectStmt, TABLE_NAME, New DBHelper.DBHelperParameter() {EquipmentIdParam, EquipmentListIdParam})
        Catch ex As Exception
            Throw New DataBaseAccessException(DataBaseAccessException.DatabaseAccessErrorType.ReadErr, ex)
        End Try
    ...
    Public Function InsertEquipment(equipmentId As Guid, companyIds As ArrayList, languageId As Guid) As Boolean

        Dim selectStmt As String = Config("/SQL/INSERT_EQUIPMENT")
        Dim ds As New DataSet
        Dim EquipmentIdParam As DBHelper.DBHelperParameter

        Try
            EquipmentIdParam = New DBHelper.DBHelperParameter(COL_NAME_EQUIPMENT_LIST_ID, equipmentId)
            DBHelper.Fetch(ds, selectStmt, TABLE_NAME, New DBHelper.DBHelperParameter() {EquipmentIdParam})
        Catch ex As Exception
            Throw New DataBaseAccessException(DataBaseAccessException.DatabaseAccessErrorType.ReadErr, ex)
        End Try
    ...
    Public Function IsListToDealer(ListCode As String, EquipmentListID As Guid) As DataSet
        ...
        Try
            ...
            Return DBHelper.Fetch(ds, selectStmt, TABLE_DEALER, New DBHelper.DBHelperParameter() {EquipmentListIdParam})
        Catch ex As Exception
            Throw New DataBaseAccessException(DataBaseAccessException.DatabaseAccessErrorType.ReadErr, ex)
        End Try
    End Function
...
#Region "Public Method"
    Public Function GetSelectedEquipmentList(EquipmentListID As Guid) As DataSet
        Dim ds As New DataSet
        Dim selectStmt As String = Config("/SQL/GetSelectedEquipmentList")
