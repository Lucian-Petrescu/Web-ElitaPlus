...
#Region "DALTestRelated Methods"
    Public Function GetLanguageId() As DataSet
        ...
        Dim selectStmt As String = Config("SQL/LOAD_LANGUAGE_ID")
    ...
    Public Function GetCompanyId() As DataSet
        ...
        Dim selectStmt As String = Config("SQL/LOAD_COMPANY_ID")
...
#Region "Public Methods"
    ...
    Public Function Load(ds As DataSet, id As Guid) As DataSet
        ...
        'End If
        Return (DBHelper.Fetch(ds, selectStmt, RISK_TYPE_TABLE_NAME, parameters))
    ...
    Public Function GetRiskTypeListTest(description As String, riskTypeEnglish As String, _
                                        riskGroupId As Guid, languageId As Guid) As DataSet
        ...
        Return DBHelper.Fetch(selectStmt, DSNAME, RISK_TYPE_LIST, parameters)

    End Function

    Public Function GetRiskTypeList(description As String, riskTypeEnglish As String, _
                                    riskGroupId As Guid, languageId As Guid, _
                                    CompanyGroupId As Guid) As DataSet

        Dim selectStmt As String = Config("/SQL/LOAD_LIST_DYNAMIC_SQL")
        ...
        If (Not (riskTypeEnglish.Equals(String.Empty))) AndAlso (FormatSearchMask(riskTypeEnglish)) Then
        ...
        If Not riskGroupId.Equals(Guid.Empty) Then
            selectStmt &= Environment.NewLine & "AND rt.risk_group_id = '" & GuidToSQLString(riskGroupId) & "'"
        End If

        If Not languageId.Equals(Guid.Empty) Then
            selectStmt &= Environment.NewLine & "AND trans_ptt.language_id = '" & GuidToSQLString(languageId) & "'"
            selectStmt &= Environment.NewLine & "AND trans_rg.language_id = '" & GuidToSQLString(languageId) & "'"
        End If

        If Not CompanyGroupId.Equals(Guid.Empty) Then
            selectStmt &= Environment.NewLine & "AND Company_Group_Id = '" & GuidToSQLString(CompanyGroupId) & "'"
        End If

        selectStmt &= Environment.NewLine & "ORDER BY UPPER(rt.description)"

        Return (DBHelper.Fetch(selectStmt, RISK_TYPE_LIST))
    ...
    Public Sub LoadRiskTypeForSoftQuestion(ds As DataSet, id As Guid)

        Dim selectStmt As String = Config("/SQL/LOADSOFTQUESTIONLIST")
        ...
        DBHelper.Fetch(ds, selectStmt, RISK_TYPE_TABLE_NAME, parameters)
    ...
    Public Sub LoadAvailableRiskTypeForSoftQuestion(ds As DataSet, companyGroupId As Guid)

        Dim selectStmt As String = Config("/SQL/LOADAVAILABLESOFTQUESTIONLIST")
    ...
    Public Function Is_TaxByProductType_Yes(countryIds As ArrayList) As Boolean
        Dim selectStmt As String = Config("/SQL/TAX_BY_PRODUCT_TYPE__YES__NUM")
        Dim whereClauseConditions As String = ""
        Dim ds As DataSet

        whereClauseConditions &= Environment.NewLine & " AND " & _
                                MiscUtil.BuildListForSql(COL_NAME_COUNTRY_ID, countryIds, False)

        selectStmt = selectStmt.Replace(DYNAMIC_WHERE_CLAUSE_PLACE_HOLDER, whereClauseConditions)

        Try
            ds = DBHelper.Fetch(selectStmt, RISK_TYPE_LIST)
            If (CInt(ds.Tables(RISK_TYPE_LIST).Rows(0).Item(0)) > 0) Then
                Return True
            Else
                Return False
            End If
...
#Region "DataAdapter Configuration"
    ...
    Protected Function ConfigureDataAdapter(con As OracleConnection) As OracleDataAdapter
        'create the data adapter and its commands
        Dim da As New OracleDataAdapter
        Dim insertStmt As String = Config("/SQL/INSERT")
        Dim updateStmt As String = Config("/SQL/UPDATE")
        Dim deleteStmt As String = Config("/SQL/DELETE")

        'associate commands to data adapter

        'insert
        da.InsertCommand = New OracleCommand(insertStmt, con)
        AssignCommonParameters(da.InsertCommand)
        ...
        AssignCommonParameters(da.UpdateCommand)
        AddUpdateAuditParameters(da.UpdateCommand)
        AddWhereParametersForUpdate(da.UpdateCommand)

        'delete
        da.DeleteCommand = New OracleCommand(deleteStmt, con)
        AddWhereParametersForUpdate(da.DeleteCommand)
...
#Region "Parameter Assignment"
    ...
    Protected Sub AssignCommonParameters(cmd As OracleCommand)
    ...
    Protected Sub AddWhereParametersForUpdate(cmd As OracleCommand)
