...
Public Class CancellationReqExceptionDAL
    Inherits DALBase
...
#Region "Constants"
    ...
    Public Const COL_LANG_ID As String = "language_id"
    ...
    Public Const EMPTY_GUID_STRING As String = "00000000000000000000000000000000"
    Public Const TRANS_SCHEDULED_DATE As String = "TRANS_SCHEDULED_DATE"
#End Region
...
#Region "Overloaded Methods"
    ...
    Public Function GetExceptionList(ByVal compGroupId As Guid, ByVal transactionType As String, ByVal mobileNumber As String, ByVal langId As Guid, ByVal transDateFrom As Date, ByVal transDateTo As Date, ByVal errorCode As String, Optional ByVal transLogHeaderId As String = "") As DataSet
        Dim selectStmt As String = Me.Config("/SQL/LOAD_LIST")
        ...
        parameters(0) = New DBHelper.DBHelperParameter(COL_LANG_ID, langId.ToByteArray)

        Dim gID As Guid
        gID = New Guid(transactionType)
        transactionType = GuidToSQLString(gID)

        If Not transactionType = EMPTY_GUID_STRING Then
            If Me.FormatSearchMask(transactionType) Then
                whereClauseConditions &= " AND " & Environment.NewLine & "UPPER(" & Me.COL_TRANS_TYPE_ID & ") " & transactionType.ToUpper
            End If
        End If

        If Me.FormatSearchMask(mobileNumber) Then
            whereClauseConditions &= " AND " & Environment.NewLine & "UPPER(" & Me.COL_MOBILE_NUMBER & ") " & mobileNumber.ToUpper
        End If

        If Not (Date.MinValue.Equals(transDateFrom)) Then
            whereClauseConditions &= " AND " & Environment.NewLine & " trunc(" & Me.TRANS_SCHEDULED_DATE & ")" & " >=  to_date('" & transDateFrom.ToString("MM/dd/yyyy HH:mm:ss") & "', 'mm-dd-yyyy hh24:mi:ss')"
        End If

        If Not (Date.MinValue.Equals(transDateTo)) Then
            whereClauseConditions &= " AND " & Environment.NewLine & " trunc(" & Me.TRANS_SCHEDULED_DATE & ")" & " <=  to_date('" & transDateTo.ToString("MM/dd/yyyy HH:mm:ss") & "', 'mm-dd-yyyy hh24:mi:ss')"
        End If
    ...
    Public Function ResendOrHideTransaction(ByVal cmd As String, ByVal transLogHeaderId As Guid, Optional ByVal newComunaValue As String = Nothing) As DBHelper.DBHelperParameter()
