...
#Region "Overloaded Methods"
    ...
    Public Function GetExceptionList(compGroupId As Guid, transactionType As String, mobileNumber As String, langId As Guid, transDateFrom As Date, transDateTo As Date, errorCode As String, Optional ByVal transLogHeaderId As String = "") As DataSet
        Dim selectStmt As String = Config("/SQL/LOAD_LIST")
        ...
        If Not transactionType = EMPTY_GUID_STRING Then
            If FormatSearchMask(transactionType) Then
                whereClauseConditions &= " AND " & Environment.NewLine & "UPPER(" & COL_TRANS_TYPE_ID & ") " & transactionType.ToUpper
            End If
        End If

        If FormatSearchMask(mobileNumber) Then
            whereClauseConditions &= " AND " & Environment.NewLine & "UPPER(" & COL_MOBILE_NUMBER & ") " & mobileNumber.ToUpper
        End If

        If Not (Date.MinValue.Equals(transDateFrom)) Then
            whereClauseConditions &= " AND " & Environment.NewLine & " trunc(" & TRANS_SCHEDULED_DATE & ")" & " >=  to_date('" & transDateFrom.ToString("MM/dd/yyyy HH:mm:ss") & "', 'mm-dd-yyyy hh24:mi:ss')"
        End If

        If Not (Date.MinValue.Equals(transDateTo)) Then
            whereClauseConditions &= " AND " & Environment.NewLine & " trunc(" & TRANS_SCHEDULED_DATE & ")" & " <=  to_date('" & transDateTo.ToString("MM/dd/yyyy HH:mm:ss") & "', 'mm-dd-yyyy hh24:mi:ss')"
        End If
    ...
    Public Function ResendOrHideTransaction(cmd As String, transLogHeaderId As Guid, Optional ByVal newComunaValue As String = Nothing) As DBHelper.DBHelperParameter()
