...
#Region "Constructors"
    ...
    Public Sub New(ByVal DealerId As Guid, ByVal ClaimTypeId As Guid)
        MyBase.New()
        Me.Dataset = New DataSet
        Me.LoadByDealerIdClaimTypeId(DealerId, ClaimTypeId)
    End Sub
    ...
    Protected Sub LoadByDealerIdClaimTypeId(ByVal DealerId As Guid, ByVal claimTypeId As Guid)
        Try
            ...
            Me.Row = Nothing
            If Me.Dataset.Tables.IndexOf(dal.TABLE_NAME) >= 0 Then
                Me.Row = Me.FindRow(claimTypeId, dal.COL_NAME_CLAIM_TYPE_ID, Me.Dataset.Tables(dal.TABLE_NAME))
            End If
            If Me.Row Is Nothing Then 'it is not in the dataset, so will bring it from the db
                dal.LoadByDealerIdClaimTypeId(Me.Dataset, DealerId, claimTypeId)
                Me.Row = Me.FindRow(claimTypeId, dal.COL_NAME_CLAIM_TYPE_ID, Me.Dataset.Tables(dal.TABLE_NAME))
            End If
            'If Me.Row Is Nothing Then
            '    Throw New DataNotFoundException
...
#Region "Properties"
    ...
    Public Property DealerId() As Guid
        ...
        Set(ByVal Value As Guid)
            CheckDeleted()
            Me.SetValue(DealerClmAproveClmtypeDAL.COL_NAME_DEALER_ID, Value)
        End Set
    End Property
    ...
    Public Property ClaimTypeId() As Guid
        ...
        Set(ByVal Value As Guid)
            CheckDeleted()
            Me.SetValue(DealerClmAproveClmtypeDAL.COL_NAME_CLAIM_TYPE_ID, Value)
        End Set
    End Property
