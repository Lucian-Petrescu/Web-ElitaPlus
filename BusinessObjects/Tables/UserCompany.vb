...
#Region "Constructors"
    ...
    Protected Sub LoadFromUser(ByVal oUserId As Guid)
        Try
            ...
            Me.Row = Nothing
            If Me.Dataset.Tables.IndexOf(dal.TABLE_NAME) >= 0 Then
                Me.Row = Me.FindRow(oUserId, dal.COL_NAME_USER_ID, Me.Dataset.Tables(dal.TABLE_NAME))
            End If
            If Me.Row Is Nothing Then 'it is not in the dataset, so will bring it from the db
                dal.LoadByUserId(Me.Dataset, oUserId)
                Me.Row = Me.FindRow(oUserId, dal.COL_NAME_USER_ID, Me.Dataset.Tables(dal.TABLE_NAME))
            End If
            If Me.Row Is Nothing Then
                Throw New DataNotFoundException
...
#Region "Properties"
    ...
    Public Property UserId() As Guid
        ...
        Set(ByVal Value As Guid)
            CheckDeleted()
            Me.SetValue(UserCompanyDAL.COL_NAME_USER_ID, Value)
        End Set
    End Property
    ...
    Public Property CompanyId() As Guid
        ...
        Set(ByVal Value As Guid)
            CheckDeleted()
            Me.SetValue(UserCompanyDAL.COL_NAME_COMPANY_ID, Value)
        End Set
    End Property

    Public Sub InitTable()
        Me.Dataset.Tables(UserCompanyDAL.TABLE_NAME).Rows.Clear()
    End Sub
